...
it('User Page', function(){
    ...
    cy.get('#user_add_firstname').type('Christian')
    ...
    cy.get('#user_add_customers').select(['3', '118'], {force:true} ) //we used {force:true} because the dropdown is getting close automatically once you a client is already selected 
    cy.get('.switch-custom').click()
    // cy.get('#select2-reports-container').click()
